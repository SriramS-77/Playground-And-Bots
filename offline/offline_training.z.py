...
# --- Configuration ---
DATA_DIR = "data"
MODEL_SAVE_PATH = "models/4/offline_rl_model_2_0.pt"

# Simulation Hyperparameters
TARGET_HUMAN_COUNT = 80
TARGET_BOT_COUNT = 0
SIMULATION_STEP_SECONDS = 10 
SESSION_CHUNK_SIZE = int(SIMULATION_STEP_SECONDS * 1000) 
TOTAL_EPISODES = 200
BATCH_SIZE = 64
TARGET_UPDATE_FREQUENCY = 200 # Global steps between target network syncs
...
try:
    ...
    for episode in range(TOTAL_EPISODES):
        update_human_bot_population(100, 500)
        ...
        while active_users:
            ...
            for user in active_users:
                ...
                h_score = get_humanity_score(mouse_data) or user.average_humanity_score
                
                # Update confusion matrix
                label = "human" if user.is_bot is False else "bot"
                pred = "human" if h_score <= 0.5 else "bot"
                conf[label]["predicted"][pred] += 1
                
                # Update user stats
                user.update_stats(h_score)
                # user.last_humanity_score = h_score
                # user.average_humanity_score = (user.average_humanity_score * user.n_humanity_score + h_score) / (user.n_humanity_score + 1)
        ...
        with open("models/4/metrics/metrics.jsonl", "a") as f:
            f.write(json.dumps(EPISODE_REWARD) + "\n")

        if (episode + 1) % 20 == 0:
            offline_rl_agent.save_model(f"models/4/offline_rl_model_2_{episode+1}.pt")
        ...
        if episode_b_step_counter > 0 and cur_score > best_score:
            ...
            offline_rl_agent.save_model(filepath=f"models/4/best_model_{best_score:.2f}.pt")
