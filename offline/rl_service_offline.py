...
class ThreatAssessor:
    ...
    def train_model(self, batch_size):
        ...
        # Decaying sampling strategy: Prioritize recent samples
        decay_factor = 0.5  # Decay rate (could be adjusted)
        # Assign weights to experiences based on their age in memory
        experience_weights = [decay_factor**(len(self.memory) - idx) for idx in range(len(self.memory))]
        
        # Normalize weights so they sum to 1
        total_weight = sum(experience_weights)
        normalized_weights = [w / total_weight for w in experience_weights]
        
        # Sample based on weighted probabilities
        batch_indices = random.choices(range(len(self.memory)), k=batch_size, weights=normalized_weights)
