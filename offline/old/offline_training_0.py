...
MODEL_SAVE_PATH = "models/offline_rl_model_1.pth"
...
TOTAL_EPISODES = 20
BATCH_SIZE = 8
TARGET_UPDATE_FREQUENCY = 10  # Update target network every 10 steps
...
def calculate_reward(user, threat_level):
    """Calculates reward and done status based on action."""
    reward = 0
    done = False
    if user.is_bot:
        if random.random() < (threat_level / 10.0):
            reward = 50.0  # Caught bot
            done = True
            user.reset_session()
        else:
            reward = -10.0  # Bot survived
...
def run_offline_training():
    ...
    for episode in range(TOTAL_EPISODES):
        ...
        while active_users:
            ...
            server_req_rate_per_min = server_total_users * (random.randint(5, 15))
            ...
            for user in active_users:
                ...
                if user.client_id in pending_experiences:
                    ...
                    # The current state s_t is the "next_state" (s') for the previous action
                    s_prime = s_t
                ...
                if user.done:
                    # The user's session is over. They don't take a new action.
                    # Their last pending experience (from t-1) has just been stored.
                    continue
                ...
                print(f"{"Bottt" if user.is_bot else "Human"} -> action: {a_t},reward {r_t}, done: {done_t}, score: {humanity_score:.2f}")
                ...
                if done_t:
                    ...
                    # The 'next_state' (s_t) is arbitrary, as 'done=True' nullifies it.
                    s_prime = s_t
                    offline_rl_agent.remember(s_t, a_t, r_t, s_prime, done_t)
                    episode_reward += r_t
                    # Do NOT add user to next_active_users
                    # Do NOT add to new_pending_experiences
                else:
            ...
            if len(offline_rl_agent.memory) > BATCH_SIZE * 10:  # Wait for a decent buffer
